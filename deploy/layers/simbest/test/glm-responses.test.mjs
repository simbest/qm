...
test("turns streamed textual tool markup into a Responses function call", async (t) => {
  const upstream = createServer(async (req, res) => {
    ...
    res.writeHead(200, { "content-type": "text/event-stream" });
    res.write(
      'data: {"choices":[{"delta":{"content":"准备执行。<function_calls><invoke name=\\"execute\\"><parameter name=\\"command\\">baidu-hotlist --limit 5</parameter></invoke></function_calls>"}}]}\n\n',
    );
    res.end("data: [DONE]\n\n");
  });
  const upstreamPort = await listen(upstream);
  ...
  assert.match(events, /baidu-hotlist --limit/);
